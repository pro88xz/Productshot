...
const GUEST_SCENE_LABEL = 'Studio White';
...
export default function TryPage() {
  ...
  async function handleFile(file: File) {
    ...
    // Upload to Supabase Storage via existing /api/upload endpoint
    setStage('uploading');
    try {
      ...
      // Now trigger the guest generation
      setStage('generating');
  ...
  function onDownloadClick() {
    // The whole point of guest mode: block the download, prompt signup
  ...
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1">
        <section className="container-prose py-12 sm:py-16 md:py-20">
          ...
          <div className="mx-auto mt-10 max-w-4xl sm:mt-14">
            ...
            {/* Error state */}
            {stage === 'error' && (
              <ErrorPanel error={errorMsg} onRetry={reset} />
            )}
          </div>
      ...
      {/* Signup modal */}
      {showSignupModal && (
        <SignupModal onClose={() => setShowSignupModal(false)} onSignup={goToSignup} />
      )}
    </div>
  );
}
      ...
      {routingMeta && (
        <div className="border-border/60 bg-muted/30 mt-6 rounded-lg border p-4 text-center sm:mt-8">
          <p className="text-muted-foreground text-xs sm:text-sm">
            Generated via{' '}
            <span className="text-foreground font-medium">{routingMeta.path}</span> path
            in{' '}
    ...
    >
      ...
      >
        ...
        <p className="text-muted-foreground mt-3 text-center text-sm">
          ...
          You&apos;ll get <span className="text-foreground font-medium">3 free credits</span>
          {' '}
          to generate more styles.
        </p>
